...
HF_TOKEN = "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"  # Secure in production
...
def evaluate_against_job(combined_texts: List[str], job: Dict) -> Dict:
    ...
    result = {
        ...
        "overall_scores": {"requirements": 0.0, "responsibilities": 0.0}
    }
    ...
    result["overall_scores"]["requirements"] = sum(req_scores) / len(req_scores) if req_scores else 0
    result["overall_scores"]["responsibilities"] = sum(resp_scores) / len(resp_scores) if resp_scores else 0
    result["overall_scores"]["combined"] = (
        result["overall_scores"]["requirements"] + result["overall_scores"]["responsibilities"]
    ) / 2
